...
public class VarNumberIOTest {

    @ParameterizedTest
    @MethodSource("intValues")
    public void positiveVarIntTest(int i) {
        ByteBuffer buffer = ByteBuffer.allocate(5);
        VarNumberIO.writePositiveVarInt(buffer, i);
        buffer.flip();
        ...
        Assertions.assertEquals(i, VarNumberIO.readPositiveVarInt(buffer));
    }
    ...
    public void varIntTest(int i) {
        ByteBuffer buffer = ByteBuffer.allocate(5);
        VarNumberIO.writeVarInt(buffer, i);
        buffer.flip();
        int dataBits = 32 - Integer.numberOfLeadingZeros(i < 0 ? ~i : i);
        int totalBits = dataBits + dataBits / 8 + 2;
        int numberOfBytes = (int) Math.ceil(totalBits / 8.0);
        Assertions.assertEquals(numberOfBytes, buffer.limit());
        Assertions.assertEquals(i, VarNumberIO.readVarInt(buffer));
    }
    ...
    public void positiveVarLongTest(long l) {
        ...
        VarNumberIO.writePositiveVarLong(buffer, l);
        buffer.flip();
        ...
        Assertions.assertEquals(numberOfBytes, buffer.limit());
        Assertions.assertEquals(l, VarNumberIO.readPositiveVarLong(buffer));
    }
    ...
    public void varLongTest(long l) {
        ByteBuffer buffer = ByteBuffer.allocate(9);
        VarNumberIO.writeVarLong(buffer, l);
        buffer.flip();
        ...
        Assertions.assertEquals(numberOfBytes, buffer.limit());
        Assertions.assertEquals(l, VarNumberIO.readVarLong(buffer));
    }
